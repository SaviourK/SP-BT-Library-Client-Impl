...
public class SessionStatePrinter {
    ...
    private final AtomicReference<TorrentSessionState> sessionState;
    private final AtomicReference<ProcessingStage> processingStage;
    ...
    public SessionStatePrinter(JLabel jLabel) {
    ...
    public void onTorrentFetched(Torrent torrent) {
        jLabel.setText(String.format("Downloading %s (%,d B)", torrent.getName(), torrent.getSize()));
    ...
    public void start() {
        ...
        jLabel.setText("Fetching metadata... Please wait");
    ...
    private void print(Torrent torrent, TorrentSessionState sessionState, ProcessingStage stage, Stats stats) {
        ...
        switch (stage) {
            case DOWNLOADING: {
                ...
                jLabel.setText(String.format(FORMAT_D, peerCount, completePercents, requiredPercents, down, up, elapsedTime, remainingTime));
                break;
            }
            case SEEDING: {
                String up = formatRate(stats.getUploadRate());
                jLabel.setText(String.format(FORMAT_S, peerCount, up));
                break;
            }
            default: {
                // ignore
